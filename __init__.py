...
class AdsEntity(Entity):
    ...
    async def async_initialize_device(
        ...
        self._state_dict[STATE_KEY_STATE] = False
...
#CHG: doesn't add the device notification, we are polling
...
#            _LOGGER.debug("Variable %s: Timeout during first update", ads_var)
    ...
    def should_poll(self):
        """CHG: this ADS module polls instead of pushing"""
        return True
...
#CHG: defune update method
    def update(self):
        if self._plctype is None:
            _LOGGER.error("update() called but device was never initialized var=%s",self._ads_var)
        value = self._ads_hub.read_by_name(self._ads_var, self._plctype)
        #_LOGGER.warning("Polling update %s: new value: %s", self._ads_var, str(value))
        self._state_dict[STATE_KEY_STATE] = value
