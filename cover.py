...
#CHG: defune update method
    def update(self):
        # check default var
        if self._ads_var is not None:
            value = self._ads_hub.read_by_name(self._ads_var, self._ads_hub.PLCTYPE_BOOL)
            #_LOGGER.warning("Cover close Polling update %s: new value: %s", self._ads_var, str(value))
            self._state_dict[STATE_KEY_STATE] = value
        # check position var
        if self._ads_var_position is not None:
            value = self._ads_hub.read_by_name(self._ads_var_position, self._ads_hub.PLCTYPE_BYTE)
            #_LOGGER.warning("Cover position Polling update %s: new value: %s", self._ads_var, str(value))
